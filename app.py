...
if view_choice == "Bản đồ":
    ...
    with st.sidebar:
        ...
        st.markdown(
            """
            <a href="http://bandocogioihoa.luagao.org/" target="_blank">
                <button style="
                    width:100%;
                    margin-bottom:10px;
                    padding:12px;
                    ...
                    font-weight:600;
                    cursor:pointer;">
                    Bản đồ Hiện trạng Cơ giới hóa
                </button>
            </a>

            <a href="https://gis.luagao.org/mapstore/#/" target="_blank">
                <button style="
                    width:100%;
                    padding:12px;
                    ...
                    font-weight:600;
                    cursor:pointer;">
                    Bản đồ Phù hợp cơ giới hóa gieo sạ
                </button>
            </a>
            """,
            unsafe_allow_html=True
...
df_indicators = pd.read_excel("data/bdcgh_indicators.xlsx", engine="openpyxl")
...
filtered_indicators = df_indicators[df_indicators["stage"] == stage_choice]
...
def plot_bar_py(df, x_var, y_var, ylab, plot_title=""):
    p = (
        ggplot(df, aes(x=x_var, y=y_var, fill=x_var))
...
# ==========================
# ✅ 4. ROW 2 — COL 2 → BIỂU ĐỒ THỰC TRẠNG ĐÁP ỨNG (% DIỆN TÍCH) THEO XÃ
# ==========================
with row2_col2:    
    st.markdown(
        f"""
        <div style="display: flex; align-items: center; margin-bottom:5px;">
            <img src="data:image/png;base64,{get_base64_bin_file(os.path.join(current_dir, "icon", "modernization.png"))}" 
                style="width: 30px; height: 30px; margin-right: 12px; vertical-align: middle;">
            <h3 style="font-size: 22px; font-weight: 200; margin: 0">
                Thực trạng đáp ứng - {stage_choice}
            </h3>
        </div>
        """,
        unsafe_allow_html=True
    ...
    if filtered_indicators.empty:
        ...
        fig = plot_bar_py(
            filtered_indicators,
            x_var = 'tênxã', 
            y_var = 'Thực trạng đáp ứng (% Diện tích)', 
            ylab = "% Diện tích",
            plot_title=""
        )
        st.pyplot(fig.draw())
...
# ==========================
# ✅ 5. ROW 3 — COL 1 → BIỂU ĐỒ CÔNG SUẤT CƠ GIỚI HÓA THEO XÃ
# ==========================
with row3_col1:    
    st.markdown(
        f"""
        <div style="display: flex; align-items: center; margin-bottom:5px;">
            <img src="data:image/png;base64,{get_base64_bin_file(os.path.join(current_dir, "icon", "connectivity.png"))}" 
                style="width: 30px; height: 30px; margin-right: 12px; vertical-align: middle;">
            <h3 style="font-size: 22px; font-weight: 200; margin: 0">
                Công suất cơ giới hóa - {stage_choice}
            </h3>
        </div>
        """,
        unsafe_allow_html=True
    ...
    if filtered_indicators.empty:
        ...
        fig = plot_bar_py(
            filtered_indicators,
            x_var = 'tênxã', 
            y_var = 'Công suất cơ giới hóa (HP/ha)', 
            ylab = "HP/ha",
            plot_title=""
        )
        st.pyplot(fig.draw())
...
# ==========================
# ✅ 6. ROW 3 — COL 2 → BIỂU ĐỒ CÔNG NĂNG CƠ GIỚI HÓA THEO XÃ
# ==========================
with row3_col2:    
    st.markdown(
        f"""
        <div style="display: flex; align-items: center; margin-bottom:5px;">
            <img src="data:image/png;base64,{get_base64_bin_file(os.path.join(current_dir, "icon", "technology.png"))}" 
                style="width: 30px; height: 30px; margin-right: 12px; vertical-align: middle;">
            <h3 style="font-size: 22px; font-weight: 200; margin: 0">
                Công năng cơ giới hóa - {stage_choice}
            </h3>
        </div>
        """,
        unsafe_allow_html=True
    ...
    if filtered_indicators.empty:
        ...
        fig = plot_bar_py(
            filtered_indicators,
            x_var = 'tênxã', 
            y_var = 'Công Năng cơ giới hóa (kWh/ha)', 
            ylab = "kWh/ha",
            plot_title=""
        )
        st.pyplot(fig.draw())
...
# ==========================
# ✅ 7. ROW 4 — COL 1 → BIỂU ĐỒ PHỤ THUỘC GIAI ĐOẠN
# ==========================
with row4_col1:
